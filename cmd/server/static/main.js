...
document.addEventListener("DOMContentLoaded", function (event)
{
	...
	{
		console.log("RerenderMath called");

		if (typeof MathJax === 'undefined')
		{
			console.warn("MathJax not available for re-rendering");
		...
		return MathJax.startup.promise.then(() =>
		{
			...
			{
				console.log("Clearing existing MathJax state and re-rendering");
				return MathJax.typesetClear().then(() =>
				{
		...
		{
			console.log("MathJax re-typeset completed successfully");
		}).catch((err) =>
		{
	...
	// Function to render math in a specific element (useful for dynamic content)
	function RerenderMathInElement(element)
	{
		console.log("RerenderMathInElement called for:", element.id || element.tagName);

		if (typeof MathJax === 'undefined')
		{
			console.warn("MathJax not available for element re-rendering");
			return Promise.resolve();
		}
		...
		return MathJax.startup.promise.then(() =>
		{
			if (MathJax.typesetPromise)
			{
				console.log("Rendering math in element:", element.id || element.tagName);
				return MathJax.typesetPromise([element]);
			}
			else
			{
				console.warn("MathJax typesetPromise not available, falling back to full render");
				return RerenderMath();
			}
		}).then(() =>
		{
			console.log("MathJax element re-typeset completed for:", element.id || element.tagName);
		}).catch((err) =>
		{
			console.error("MathJax typeset failed for element:", err);
		});
	}
	...
	function DoOrbitPanel(obj)
	{
		try
		{
			console.log("DoOrbitPanel called with:", obj);
			console.log("obj.Content:", obj.Content);
			console.log("obj.Content.length:", obj.Content ? obj.Content.length : "No Content");

			RerenderMath();
			let section = document.querySelector("main");
			console.log("Found main section:", section);

			if (!section)
			{
				throw new Error("Could not find main section");
			}
			...
			if (obj == null)
			{
				console.log("obj is null, returning");
				return;
			}

			if (!obj.Content || obj.Content === null)
			{
				console.log("obj.Content is null/undefined");
				panel.textContent = "No content available";
				return;
			}
			...
			{
				console.log("obj.Content is not an array or length is 0, showing 'No result'");
				panel.textContent = "No result";
				return;
			}

			console.log("Processing", obj.Content.length, "content items");

			let last_node_event = obj.Content[0];

			for (let i = 0; i < obj.Content.length && i < obj.Content.length; i++)
			{
				let node_event = obj.Content[i];
				if (!node_event)
				{
					console.warn("Skipping null/undefined node_event at index", i);
					continue;
				}
				console.log("Processing node_event", i, ":", node_event);
				...
				{
					console.error("Error processing node_event", i, ":", nodeError);
					// Continue with next node instead of failing completely
				}
			}

			console.log("DoOrbitPanel completed successfully");

			// Re-render math in the newly added content with a small delay to ensure DOM is ready
			setTimeout(() =>
			{
				RerenderMathInElement(panel);
			}, 100);
		} catch (error)
		{
			console.error("Error in DoOrbitPanel:", error);
			DisplayError("Error in DoOrbitPanel: " + error.message);
		}
	}
	...
	function DoPageMapPanel(obj)
	{
		...
		// Re-render math in the newly added content after notes are printed with delay
		setTimeout(() =>
		{
			RerenderMathInElement(panel);
		}, 100);
	...
	function DoTOCPanel(obj)
	{
		...
		for (let chpblk of obj.Content)
		{
			...
			{
				console.warn("Missing XYZ coordinates for chapter:", chpblk.Chapter);
			}
