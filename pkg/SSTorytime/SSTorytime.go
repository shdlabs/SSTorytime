...
func UploadContextToDB(sst PoSST, contextstring string, ptr ContextPtr) ContextPtr {
	...
	row, err := sst.DB.Query(qstr)
	if err != nil {
		slog.Error("FAILED", "query", qstr, "error", err)
	}
	...
	if row != nil {
		for row.Next() {
			if err := row.Scan(&cptr); err != nil {
				slog.Error("FAILED", "scan", cptr, "error", err)
			}
		}
		defer row.Close()
	}
...
func GetDBNodePtrMatchingNCCS(sst PoSST, nm, chap string, cn []string, arrow []ArrowPtr, seq bool, limit int) []NodePtr {
	...
	slog.Info("GetDBNodePtrMatchingNCCS starting",
		"name", nm,
		"chapter", chap,
		"context_count", len(cn),
		"arrows_count", len(arrow),
		"seq", seq,
		"limit", limit)
	...
	slog.Debug("WHERE clause built", "duration_ms", time.Since(whereStart).Milliseconds())

	qstr := fmt.Sprintf("SELECT NPtr FROM Node WHERE %s ORDER BY L,NPtr LIMIT %d", whereClause, limit)

	queryStart := time.Now()
	slog.Debug("Sending DB query", "query_type", "GetDBNodePtrMatchingNCCS", "timestamp", queryStart.Format("15:04:05.000"))
	row, err := sst.DB.Query(qstr)
	queryEnd := time.Now()
	slog.Debug("DB query response received", "query_type", "GetDBNodePtrMatchingNCCS", "timestamp", queryEnd.Format("15:04:05.000"), "duration_ms", queryEnd.Sub(queryStart).Milliseconds())

	if err != nil {
		slog.Error("GetNodePtrMatchingNCC query failed", "error", err, "query", qstr)
	...
	for row.Next() {
		err = row.Scan(&whole)
		if err != nil {
			slog.Debug("Error scanning row in GetDBNodePtrMatchingNCCS", "error", err)
			continue
		}
	...
	if err = row.Err(); err != nil {
		slog.Debug("Row iteration error in GetDBNodePtrMatchingNCCS", "error", err)
	}

	slog.Debug("Results scanned", "count", len(retval), "duration_ms", time.Since(scanStart).Milliseconds())
...
func NodeWhereString(name, chap string, context []string, arrow []ArrowPtr, seq bool) string {
	start := time.Now()
	defer func() {
		slog.Debug("NodeWhereString completed", "duration_ms", time.Since(start).Milliseconds())
	}()
...
func GetDBChaptersMatchingName(sst PoSST, src string) []string {
	...
	queryStart := time.Now()
	slog.Debug("Sending DB query", "query_type", "GetDBChaptersMatchingName", "timestamp", queryStart.Format("15:04:05.000"))
	row, err := sst.DB.Query(qstr)
	queryEnd := time.Now()
	slog.Debug("DB query response received", "query_type", "GetDBChaptersMatchingName", "timestamp", queryEnd.Format("15:04:05.000"), "duration_ms", queryEnd.Sub(queryStart).Milliseconds())

	if err != nil {
		slog.Error("GetDBChaptersMatchingName query failed", "error", err, "query", qstr)
	...
	for row.Next() {
		err = row.Scan(&whole)
		if err != nil {
			slog.Debug("Error scanning row in GetDBChaptersMatchingName", "error", err)
			continue
		}
	...
	if err = row.Err(); err != nil {
		slog.Debug("Row iteration error in GetDBChaptersMatchingName", "error", err)
	}
...
func GetDBContextByName(sst PoSST, src string) (string, ContextPtr) {
	...
	queryStart := time.Now()
	slog.Debug("Sending DB query", "query_type", "GetDBContextByName", "timestamp", queryStart.Format("15:04:05.000"))
	row, err := sst.DB.Query(qstr)
	queryEnd := time.Now()
	slog.Debug("DB query response received", "query_type", "GetDBContextByName", "timestamp", queryEnd.Format("15:04:05.000"), "duration_ms", queryEnd.Sub(queryStart).Milliseconds())

	if err != nil {
		slog.Error("GetDBContextByName query failed", "error", err, "query", qstr)
	...
	for row.Next() {
		...
		if err != nil {
			slog.Debug("Error scanning row in GetDBContextByName", "error", err)
			continue
		}
	}

	if err = row.Err(); err != nil {
		slog.Debug("Row iteration error in GetDBContextByName", "error", err)
	}
...
func GetDBContextByPtr(sst PoSST, ptr ContextPtr) (string, ContextPtr) {
	...
	queryStart := time.Now()
	slog.Debug("Sending DB query", "query_type", "GetDBContextByPtr", "timestamp", queryStart.Format("15:04:05.000"))
	row, err := sst.DB.Query(qstr)
	queryEnd := time.Now()
	slog.Debug("DB query response received", "query_type", "GetDBContextByPtr", "timestamp", queryEnd.Format("15:04:05.000"), "duration_ms", queryEnd.Sub(queryStart).Milliseconds())

	if err != nil {
		slog.Error("GetDBContextByPtr query failed", "error", err, "query", qstr)
	...
	for row.Next() {
		...
		if err != nil {
			slog.Debug("Error scanning row in GetDBContextByPtr", "error", err)
			continue
		}
	}

	if err = row.Err(); err != nil {
		slog.Debug("Row iteration error in GetDBContextByPtr", "error", err)
	}
...
func GetDBNodeByNodePtr(sst PoSST, db_nptr NodePtr) Node {
	...
	if cached {
		slog.Debug("GetDBNodeByNodePtr cache hit", "nptr", db_nptr)
	...
	if err != nil {
		slog.Error("GetDBNodeByNodePointer query failed", "error", err, "nptr", db_nptr, "query", qstr)
	...
	for row.Next() {
		...
		if err != nil {
			slog.Debug("Error scanning row in GetDBNodeByNodePtr", "error", err, "nptr", db_nptr)
			continue
		}
	...
	if err = row.Err(); err != nil {
		slog.Debug("Row iteration error in GetDBNodeByNodePtr", "error", err, "nptr", db_nptr)
	}
...
func GetDBSingletonBySTType(sst PoSST, sttypes []int, chap string, cn []string) ([]NodePtr, []NodePtr) {
	...
	queryStart := time.Now()
	slog.Debug("Sending DB query", "query_type", "GetDBSingletonBySTType (sources)", "timestamp", queryStart.Format("15:04:05.000"))
	row, err := sst.DB.Query(qstr)
	queryEnd := time.Now()
	slog.Debug("DB query response received", "query_type", "GetDBSingletonBySTType (sources)", "timestamp", queryEnd.Format("15:04:05.000"), "duration_ms", queryEnd.Sub(queryStart).Milliseconds())

	if err != nil {
		slog.Error("GetDBSingletonBySTType (sources) query failed", "error", err, "query", qstr)
	...
	for row.Next() {
		...
		err = row.Scan(&nstr)
		if err != nil {
			slog.Debug("Error scanning row in GetDBSingletonBySTType (sources)", "error", err)
	...
	if err = row.Err(); err != nil {
		slog.Debug("Row iteration error in GetDBSingletonBySTType (sources)", "error", err)
	}
	...
	queryStart = time.Now()
	slog.Debug("Sending DB query", "query_type", "GetDBSingletonBySTType (sinks)", "timestamp", queryStart.Format("15:04:05.000"))
	row2, err := sst.DB.Query(qstr)
	queryEnd = time.Now()
	slog.Debug("DB query response received", "query_type", "GetDBSingletonBySTType (sinks)", "timestamp", queryEnd.Format("15:04:05.000"), "duration_ms", queryEnd.Sub(queryStart).Milliseconds())

	if err != nil {
		slog.Error("GetDBSingletonBySTType (sinks) query failed", "error", err, "query", qstr)
	...
	for row2.Next() {
		...
		err = row2.Scan(&nstr)
		if err != nil {
			slog.Debug("Error scanning row in GetDBSingletonBySTType (sinks)", "error", err)
			fmt.Println("Error scanning sql data case", dim, "gave error", err, qstr)
			continue
		}
	...
	if err = row2.Err(); err != nil {
		slog.Debug("Row iteration error in GetDBSingletonBySTType (sinks)", "error", err)
	}
...
func GetDBArrowByPtr(sst PoSST, arrowptr ArrowPtr) ArrowDirectory {
	...
	if int(arrowptr) > len(ARROW_DIRECTORY) {
		slog.Debug("GetDBArrowByPtr downloading arrows", "arrowptr", arrowptr)
		DownloadArrowsFromDB(sst)
	}
...
func ArrowPtrFromArrowsNames(sst PoSST, arrows []string) ([]ArrowPtr, []int) {
	start := time.Now()
	defer func() {
		slog.Debug("ArrowPtrFromArrowsNames completed",
			"input_count", len(arrows),
			"duration_ms", time.Since(start).Milliseconds())
	}()
...
func SolveNodePtrs(sst PoSST, nodenames []string, search SearchParameters, arr []ArrowPtr, limit int) []NodePtr {
	start := time.Now()
	defer func() {
		slog.Debug("SolveNodePtrs completed", "nodenames_count", len(nodenames), "result_count", "deferred", "duration_ms", time.Since(start).Milliseconds())
	}()
	...
	slog.Info("SolveNodePtrs starting", "nodenames", nodenames, "chapter", chap, "context_count", len(cntx), "arrows_count", len(arr), "limit", limit)
...
func GetDBPageMap(sst PoSST, chap string, cn []string, page int) []PageMap {
	start := time.Now()
	defer func() {
		slog.Debug("GetDBPageMap completed",
			"chapter", chap,
			"page", page,
			"duration_ms", time.Since(start).Milliseconds())
	}()
	...
	row, err := sst.DB.Query(qstr)
	if err != nil {
		slog.Error("GetDBPageMap query failed", "error", err, "query", qstr)
	...
	for row.Next() {
		...
		if err != nil {
			slog.Debug("Error scanning row in GetDBPageMap", "error", err)
	...
	if err = row.Err(); err != nil {
		slog.Debug("Row iteration error in GetDBPageMap", "error", err)
	}
...
func GetEntireConePathsAsLinks(sst PoSST, orientation string, start NodePtr, depth int, limit int) ([][]Link, int) {
	...
	row, err := sst.DB.Query(qstr)
	if err != nil {
		slog.Error("AllPathsAsLinks query failed", "error", err, "query", qstr)
	...
	for row.Next() {
		err = row.Scan(&whole)
		if err != nil {
			slog.Debug("Error scanning row in GetEntireConePathsAsLinks", "error", err)
			continue
		}
		retval = ParseLinkPath(whole)
	}
	...
	if err = row.Err(); err != nil {
		slog.Debug("Row iteration error in GetEntireConePathsAsLinks", "error", err)
	}
...
func DownloadArrowsFromDB(sst PoSST) {
	...
	queryStart := time.Now()
	slog.Debug("Sending DB query", "query_type", "DownloadArrowsFromDB", "timestamp", queryStart.Format("15:04:05.000"))
	row, err := sst.DB.Query(qstr)
	queryEnd := time.Now()
	slog.Debug("DB query response received", "query_type", "DownloadArrowsFromDB", "timestamp", queryEnd.Format("15:04:05.000"), "duration_ms", queryEnd.Sub(queryStart).Milliseconds())

	if err != nil {
		slog.Error("DownloadArrowsFromDB query failed", "error", err, "query", qstr)
	...
	for row.Next() {
		...
		if err != nil {
			slog.Debug("Error scanning row in DownloadArrowsFromDB", "error", err)
			continue
		}
		ad.STAindex = staidx
		ad.Long = long
	...
	if err = row.Err(); err != nil {
		slog.Debug("Row iteration error in DownloadArrowsFromDB", "error", err)
	}
	...
	queryStart = time.Now()
	slog.Debug("Sending DB query", "query_type", "DownloadArrowsFromDB (inverses)", "timestamp", queryStart.Format("15:04:05.000"))
	row2, err := sst.DB.Query(qstr)
	queryEnd = time.Now()
	slog.Debug("DB query response received", "query_type", "DownloadArrowsFromDB (inverses)", "timestamp", queryEnd.Format("15:04:05.000"), "duration_ms", queryEnd.Sub(queryStart).Milliseconds())

	if err != nil {
		slog.Error("DownloadArrowsFromDB (inverses) query failed", "error", err, "query", qstr)
	...
	for row2.Next() {
		...
		err = row2.Scan(&plus, &minus)
		if err != nil {
			slog.Debug("Error scanning row in DownloadArrowsFromDB (inverses)", "error", err)
			fmt.Println("QUERY Download Arrows Failed", err)
			continue
		}
	...
	if err = row2.Err(); err != nil {
		slog.Debug("Row iteration error in DownloadArrowsFromDB (inverses)", "error", err)
	}
}
...
func DownloadContextsFromDB(sst PoSST) {
	...
	queryStart := time.Now()
	slog.Debug("Sending DB query", "query_type", "DownloadContextsFromDB", "timestamp", queryStart.Format("15:04:05.000"))
	row, err := sst.DB.Query(qstr)
	queryEnd := time.Now()
	slog.Debug("DB query response received", "query_type", "DownloadContextsFromDB", "timestamp", queryEnd.Format("15:04:05.000"), "duration_ms", queryEnd.Sub(queryStart).Milliseconds())

	if err != nil {
		slog.Error("DownloadContextsFromDB query failed", "error", err, "query", qstr)
		fmt.Println("QUERY Download Arrows Failed", err)
		return
	}
	defer row.Close()
	...
	for row.Next() {
		...
		if err != nil {
			slog.Debug("Error scanning row in DownloadContextsFromDB", "error", err)
			continue
		}
	...
	if err = row.Err(); err != nil {
		slog.Debug("Row iteration error in DownloadContextsFromDB", "error", err)
	}
}
...
func WaveFrontsOverlap(sst PoSST, left_paths, right_paths [][]Link, Lnum, Rnum, ldepth, rdepth int) ([][]Link, [][]Link) {
	start := time.Now()
	defer func() {
		slog.Debug("WaveFrontsOverlap completed",
			"left_paths", Lnum,
			"right_paths", Rnum,
			"ldepth", ldepth,
			"rdepth", rdepth,
...
func (o Orbit) Equals(other Orbit) bool {
	return o.Dst == other.Dst && o.Arrow == other.Arrow && o.Text == other.Text
}

func IdempAddNote(list []Orbit, item Orbit) []Orbit {
	for o := range list {
		if list[o].Equals(item) {
			return list
		}
	}
	// If called in a tight loop, consider preallocating 'list' with enough capacity outside this function.
...
func GetSequenceContainers(sst PoSST, nodeptrs []NodePtr, arrowptrs []ArrowPtr, sttypes []int, limit int) []Story {
	start := time.Now()
	defer func() {
		slog.Debug("GetSequenceContainers completed",
			"nodes_count", len(nodeptrs),
			"arrows_count", len(arrowptrs),
			"duration_ms", time.Since(start).Milliseconds())
	}()
...
func AssembleSatellitesBySTtype(sst PoSST, stindex int, satellite []Orbit, sweep [][]Link, exclude_vector string, probe_radius int, limit int) []Orbit {
	start := time.Now()
	defer func() {
		slog.Debug("AssembleSatellitesBySTtype completed",
			"stindex", stindex,
			"duration_ms", time.Since(start).Milliseconds())
	}()
...
func GetLastSawSection(sst PoSST) []LastSeen {
	qstr := "SELECT section,nptr,last,freq,delta as pdelta,EXTRACT(EPOCH FROM NOW()-last) as ndelta from Lastseen ORDER BY section"

	row, err := sst.DB.Query(qstr)
	if err != nil {
		slog.Error("GetLastSawSection failed", "query", qstr, "error", err)
		return nil
	}
...
func CommitContextToken(token string, now int64, key string) {
	...
	if obs.Freq > 1 {
		...
		slog.Info("last saw", "token", token, "at", pr, "status", okey)
	}
...
func GetChaptersByChapContext(sst PoSST, chap string, cn []string, limit int) map[string][]string {
	start := time.Now()
	defer func() {
		slog.Debug("GetChaptersByChapContext completed",
			"chapter", chap,
			"context_count", len(cn),
			"duration_ms", time.Since(start).Milliseconds())
	}()
	...
	row, err := sst.DB.Query(qstr)
	if err != nil {
		slog.Error("GetChaptersByChapContext query failed", "error", err, "query", qstr)
	...
	for row.Next() {
		...
		if err != nil {
			slog.Debug("Error scanning row in GetChaptersByChapContext", "error", err)
			continue
		}
		...
		for c := 0; c < len(chps); c++ {
			...
			if len(toc) == limit {
				if err = row.Err(); err != nil {
					slog.Debug("Row iteration error in GetChaptersByChapContext", "error", err)
				}
				return toc
			}
...
func DecodeSearchField(cmd string) SearchParameters {
	start := time.Now()
	defer func() {
		slog.Debug("DecodeSearchField completed",
