...
func OverrideCredentials(u, p, d string) (string, string, string) {
	...
	if err != nil && len(dirname) > 1 {
		slog.Error("Unable to determine user's home directory", "error", err)
		os.Exit(-1)
	}
...
func Configure(ctx PoSST, load_arrows bool) {
	...
	if !CreateType(ctx, NODEPTR_TYPE) {
		slog.Error("Unable to create type as", "type", NODEPTR_TYPE)
		os.Exit(-1)
	}

	if !CreateType(ctx, LINK_TYPE) {
		slog.Error("Unable to create type as", "type", LINK_TYPE)
		os.Exit(-1)
	}

	if !CreateType(ctx, APPOINTMENT_TYPE) {
		slog.Error("Unable to create type as", "type", APPOINTMENT_TYPE)
		os.Exit(-1)
	}

	if !CreateTable(ctx, CONTEXT_DIRECTORY_TABLE) {
		slog.Error("Unable to create table as", "table", CONTEXT_DIRECTORY_TABLE)
		os.Exit(-1)
	}
	...
	if !CreateTable(ctx, PAGEMAP_TABLE) {
		slog.Error("Unable to create table as", "table", PAGEMAP_TABLE)
		os.Exit(-1)
	}

	if !CreateTable(ctx, NODE_TABLE) {
		slog.Error("Unable to create table as", "table", NODE_TABLE)
		os.Exit(-1)
	}

	if !CreateTable(ctx, ARROW_INVERSES_TABLE) {
		slog.Error("Unable to create table as", "table", ARROW_INVERSES_TABLE)
		os.Exit(-1)
	}

	if !CreateTable(ctx, ARROW_DIRECTORY_TABLE) {
		slog.Error("Unable to create table as", "table", ARROW_DIRECTORY_TABLE)
		os.Exit(-1)
	}

	if !CreateTable(ctx, LASTSEEN_TABLE) {
		slog.Error("Unable to create table as", "table", LASTSEEN_TABLE)
		os.Exit(-1)
	}
	...
	if ok {
		...
		case N1GRAM:
			for key := range NODE_DIRECTORY.N1grams {
				if strings.EqualFold(key, event.S) {
...
func IdempAddChapterSeqToNode(class int, cptr ClassedNodePtr, chap string, seq bool) {
	...
	var node Node = UpdateSeqStatus(class, cptr, seq)
...
func UpdateSeqStatus(class int, cptr ClassedNodePtr, seq bool) Node {
	...
	slog.Error("Non existent node class (shouldn't happen)", "error", errors.New("invalid node class"))
	// os.Exit(-1)
	var dummy Node
	return dummy
}
...
func LinearFindText(in []Node, event Node, ignore_caps bool) (ClassedNodePtr, bool) {
	for i := 0; i < len(in); i++ {
		...
		if ignore_caps {
			if strings.EqualFold(in[i].S, event.S) {
...
func GraphToDB(ctx PoSST, wait_counter bool) {
	...
	slog.Info("Storing Arrows...", "function", "GraphToDB")

	ctx.DB.QueryRow("drop table ArrowDirectory")
	ctx.DB.QueryRow("drop table ArrowInverses")

	if !CreateTable(ctx, ARROW_INVERSES_TABLE) {
		slog.Error("Unable to create table", "table", ARROW_INVERSES_TABLE)
		os.Exit(-1)
	}
	if !CreateTable(ctx, ARROW_DIRECTORY_TABLE) {
		slog.Error("Unable to create table", "table", ARROW_DIRECTORY_TABLE)
		os.Exit(-1)
	}
	...
	slog.Info("Storing inverse Arrows...", "function", "GraphToDB")

	for arrow := range INVERSE_ARROWS {
		UploadInverseArrowToDB(ctx, ArrowPtr(arrow))
	}

	slog.Info("Storing contexts...", "function", "GraphToDB")

	UploadContextsToDB(ctx)

	slog.Info("Storing page map...", "function", "GraphToDB")
	...
	slog.Info("Indexing ....", "function", "GraphToDB")
	...
	slog.Info("Finally done!", "function", "GraphToDB")
}
...
func HubJoin(ctx PoSST, name, chap string, nptrs []NodePtr, arrow string, context []string, weight []float32) Node {
	...
	if nptrs == nil {
		slog.Error("Call to HubJoin with a null list of pointers", "error", errors.New("null pointer list"))
		os.Exit(-1)
	}
	...
	if len(nptrs) != len(weight) {
		slog.Error("Call to HubJoin with inconsistent node/weight pointer arrays", "nptrs", len(nptrs), "weight", len(weight))
		os.Exit(-1)
	}
...
func IdempDBAddNode(ctx PoSST, n Node) Node {
	...
	if err != nil {
		...
		if errors.Is(err, sql.ErrNoRows) {
			slog.Error("No rows returned when adding node", "query", qstr)
		}
		if errors.Is(err, sql.ErrConnDone) {
			slog.Error("Database connection done error when adding node", "query", qstr)
		}
		if errors.Is(err, sql.ErrTxDone) {
			slog.Error("Database key error when adding node", "query", qstr)
		}
		if strings.Contains(s, "duplicate key") {
			slog.Error(s, "FAILED", "query", qstr, "error", err)
		}
		return n
	}
...
func IdempDBAddLink(ctx PoSST, from Node, link Link, to Node) {
	...
	if frptr == toptr {
		slog.Error("Self-loops are not allowed", "from", from.S, "fromNode", from, "link", link, "toNode", to)
		os.Exit(-1)
	}

	if link.Arr < 0 || len(ARROW_DIRECTORY) == 0 {
		slog.Error("No arrows have yet been defined, so you can't rely on the arrow names")
		os.Exit(-1)
	}

	if link.Wgt == 0 {
		slog.Error("Attempt to register a link with zero weight is pointless", "link", link)
		os.Exit(-1)
	}
...
func AppendDBLinkToNode(ctx PoSST, n1ptr NodePtr, lnk Link, sttype int) bool {
	...
	if sttype < -EXPRESS || sttype > EXPRESS {
		slog.Error(ERR_ST_OUT_OF_BOUNDS, "sttype", sttype)
		os.Exit(-1)
	}
	...
	row, err := ctx.DB.Query(qstr)
	if err != nil {
		slog.Error("Failed to append", "error", err, "query", qstr)
		return false
	}
...
func DefineStoredFunctions(ctx PoSST) {
	...
	row, err := ctx.DB.Query(qstr)
	if err != nil {
		slog.Error("Error defining postgres function:", "query", qstr, "error", err)
	}
