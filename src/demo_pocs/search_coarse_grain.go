...
func main() {
	...
	fmt.Println("Look for coarse grains",len(solutions))
...
func Together(supernodes [][]SST.NodePtr,n1 SST.NodePtr,n2 SST.NodePtr) [][]SST.NodePtr {

        // supernodes [snode][member]

	if len(supernodes) == 0 {
		var newsuper []SST.NodePtr
		newsuper = append(newsuper,n1)
		newsuper = append(newsuper,n2)
		supernodes = append(supernodes,newsuper)
		return supernodes
	}

	for i := range supernodes {
		for j := range supernodes[i] {
			if supernodes[i][j] == n1 {
				if NotIn(supernodes[i],n2) {
					supernodes[i] = append(supernodes[i],n2)
				}
				return supernodes
			}

			if supernodes[i][j] == n2 {
				if NotIn(supernodes[i],n1) {
					supernodes[i] = append(supernodes[i],n1)
				}
				return supernodes
			}
		}
...
func NotIn(list []SST.NodePtr,node SST.NodePtr) bool {
