document.addEventListener("DOMContentLoaded", (event) => {
  ...
  function populateCommandSuggestions() {
    const datalist = document.getElementById("search-suggestions");
    if (!datalist) return;

    SEARCH_COMMANDS.forEach((command) => {
  ...
  function saveSearchToHistory(query) {
    ...
    // 2. Remove any previous instance of the same query to avoid duplicates
    history = history.filter((item) => item !== query);

    // 3. Add the new query to the front of the array
    history.unshift(query);

    // 4. Limit the history to a maximum size
  ...
  function DisplayError(message) {
    ...
    main.innerHTML = `<div class="text-center p-8 bg-red-100 dark:bg-red-900/50
                text-red-700 dark:text-red-300 rounded-lg shadow-md 
        ...
        if (i % 2 == 0) {
          ...
          if (str.includes("\n")) {
            ...
            if (str.length < 20) {
              ...
              // text.style.paddingInline = "O.5rem";
            }
      ...
      for (let i = 0; i < array[line].length; i++) {
        ...
        if (i % 2 == 0) {
          ...
          if (i == 0 && str == last_line_start) {
            ...
            subline.className = "line-view";
            child.append(subline);
            let text = document.createElement("span");
            text.id = "ditto";
            text.textContent = ' . . . .  .   " . . . . .   ';
            // text.style.fontFamily = "Times New Roman";
            // text.style.padding = "1ch";
            ...
            subline = document.createElement("div");
            // subline.setAttribute("class", "line-view");
            child.append(subline);
          }
