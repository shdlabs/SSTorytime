...
class SSTCanvas3D
{
  ...
  horizon(x, y, z)
  {
    return Math.sqrt((x - this.obsX) * (x - this.obsX) + (y - this.obsY) * (y - this.obsY) + (z - this.obsZ) * (z - this.obsZ));
  }
  ...
  alpha(x, y, z)
  {
    let alpha = 1.5 / Math.sqrt(
      (x - this.obsX) * (x - this.obsX) + (y - this.obsY) * (y - this.obsY) + (z - this.obsZ) * (z - this.obsZ),
    );
  ...
  transformY(x, y, z)
  {
    const scale = (this.scale * this.width) / (1 + 1.4 * this.horizon(x, y, z));

    return (this.height - this.orgY - scale * (y + z * Math.sin(this.theta) - x * Math.sin(this.theta)));
  }
  ...
  drawGrid(centerX, centerZ, length)
  {
    ...
    {
      // Parallel to X axis
      this.drawLine3D(centerX - length / 2, 0, centerZ + i, centerX + length / 2, 0, centerZ + i, "oklch(27.1% 0.000 0.0)", 0.2 * this.mob);
      // Parallel to Z axis
      this.drawLine3D(centerX + i, 0, centerZ - length / 2, centerX + i, 0, centerZ + length / 2, "oklch(25.1% 0.009 0.0)", 0.2 * this.mob); // Parallel to X axis
    }

    // Draw coordinate axes
    this.drawLine3D(-length / 2, 0, 0, length / 2, 0, 0, "oklch(59.1% 0.293 322.896)", 2 * this.mob);
    this.drawLine3D(0, 0, -length / 2, 0, 0, length / 2, "oklch(58.8% 0.158 241.966)", 2 * this.mob);
    this.drawLine3D(0, -length / 2, 0, 0, length / 2, 0, "oklch(69.6% 0.17 162.48)", 2 * this.mob);
  ...
  drawNode(x, y, z, radius, colorInner, colorOuter)
  {
    ...
    const screenY = this.transformY(x, y, z);
    const adjustedRadius = (radius * 1.7) / this.horizon(x, y, z);

    // Create gradient
    const grad = this.ctx.createLinearGradient(screenX, screenY, screenX + adjustedRadius, screenY + adjustedRadius);
  ...
  drawArrow(x0, y0, z0, x1, y1, z1, color, thickness)
  {
    ...
    // Draw arrowhead
    this.ctx.moveTo(toX - nodeRadius * Math.cos(angle), toY - nodeRadius * Math.sin(angle));
    this.ctx.lineTo(toX - headLen * Math.cos(angle - headAngle), toY - headLen * Math.sin(angle - headAngle));
    this.ctx.moveTo(toX - nodeRadius * Math.cos(angle), toY - nodeRadius * Math.sin(angle));
    this.ctx.lineTo(toX - headLen * Math.cos(angle + headAngle), toY - headLen * Math.sin(angle + headAngle));
    this.ctx.stroke();
    this.ctx.restore();
  ...
  plotGraphics(event, lastEvent)
  {
    ...
    this.drawLabel(tx, ty, tz, event.Text.slice(0, 25), 12, "lightgrey");
  ...
  renderOrbits(event)
  {
    const orbitRenderers = [
      {
        index: this.ST_INDICES.Il1,
        nodeType: "event",
        arrowFunc: "drawLeadsToArrow",
        direction: "outgoing",
      },
      {
        index: this.ST_INDICES.Im1,
        nodeType: "event",
        arrowFunc: "drawLeadsToArrow",
        direction: "incoming",
      },
      {
        index: this.ST_INDICES.Ic2,
        nodeType: "thing",
        arrowFunc: "drawContainsArrow",
        direction: "outgoing",
      },
      {
        index: this.ST_INDICES.Im2,
        nodeType: "thing",
        arrowFunc: "drawContainsArrow",
        direction: "incoming",
      },
      {
        index: this.ST_INDICES.Ie3,
        nodeType: "concept",
        arrowFunc: "drawExpressesArrow",
        direction: "outgoing",
      },
      {
        index: this.ST_INDICES.Im3,
        nodeType: "concept",
        arrowFunc: "drawExpressesArrow",
        direction: "incoming",
      },
      {
        index: this.ST_INDICES.In0,
        nodeType: "event",
        arrowFunc: "drawNearArrow",
        direction: "outgoing",
      },
    ];
    ...
    orbitRenderers.forEach((renderer) =>
    {
      ...
      orbit.forEach((neighbor) =>
      {
        ...
        {
          this[renderer.arrowFunc](neighbor.OOO.X, neighbor.OOO.Y, neighbor.OOO.Z, neighbor.XYZ.X, neighbor.XYZ.Y, neighbor.XYZ.Z);
        } else
        {
          this[renderer.arrowFunc](neighbor.XYZ.X, neighbor.XYZ.Y, neighbor.XYZ.Z, neighbor.OOO.X, neighbor.OOO.Y, neighbor.OOO.Z);
        }
      });
    });
  }
